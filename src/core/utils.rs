...
fn get_absolute_path(path: &Path) -> PathBuf {
    if path.is_absolute() {
        ...
        return fs::canonicalize(path_buf).unwrap();
    }
}

#[inline(always)]
pub fn url_builder_boards() -> String {
    format!("{}{}{}", URL_THREAD, "boards", FORMAT_JSON)
}

#[inline(always)]
pub fn url_builder_catalog(board: &str) -> String {
    format!("{}{}{}{}", URL_THREAD, board, "/catalog", FORMAT_JSON)
}

#[inline(always)]
pub fn url_builder_thread(board: &str, thread: &str) -> String {
    format!(
        "{}{}{}{}{}",
...
pub fn create_media_path(dir: &Path, tim: i64, extension: &str) -> PathBuf {
    let file = format!("{}{}", tim, extension);
    let absolute_path = get_absolute_path(dir);

    absolute_path.join(file)
}

pub fn create_media_path_f(dir: &Path, filename: &str, extension: &str) -> PathBuf {
