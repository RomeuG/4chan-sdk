...
inline auto get_thread(std::string const& board, std::string const& thread) -> channer::json::Thread
{
    ...
    return channer::json::get_thread(json, board);
}

inline auto get_thread_json(std::string const& board, std::string const& thread) -> std::string
{
    auto website = channer::endpoints::URL_THREAD + board + channer::endpoints::TYPE_THREAD + thread + channer::endpoints::FORMAT_JSON;
    auto download = channer::req::download_json(website.c_str());
...
inline auto get_catalog(std::string const& board) -> channer::json::Catalog
{
#ifndef MOCKDATA
...
inline auto get_catalog_json(std::string const& board) -> std::string
{
#ifndef MOCKDATA
    auto website = channer::endpoints::URL_THREAD + board + "/catalog" + channer::endpoints::FORMAT_JSON;
    auto download = channer::req::download_json(website.c_str());
...
inline auto get_boards() -> channer::json::Boards
{
#ifndef MOCKDATA
...
inline auto search_board(channer::json::Board const& desired) -> std::vector<channer::json::Board>
{
...
inline auto download_media(std::string const& board, long long const tim, std::string const& extension, std::string const& filename, std::filesystem::path const& path) -> bool
{
...
inline auto download_media(std::string const& board, long long const tim, std::string const& extension, std::filesystem::path const& path) -> bool
{
    if (!std::filesystem::exists(path)) {
        if (!std::filesystem::create_directory(path)) {
            throw std::runtime_error("Error creating directory");
...
inline auto download_media(std::string const& board, std::string const& thread,
...
inline auto download_media(std::string const& url, std::filesystem::path const& path) -> bool
{
